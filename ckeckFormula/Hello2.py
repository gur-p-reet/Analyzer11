...
def getGroupValancy(groupName):
    global times
    times = 0
    grouplength = groupName.__len__()
    valancyList=[]
    if groupBonds (groupName) !=0:
        groupValancy = groupBonds (groupName)
    else:
        i = 0
        char1 = groupName[0]
        groupValancy = elementBonds(char1)

        while i in range(1, grouplength - 1):
            char = groupName[i]
            if groupName[i].isalpha():
                bonds = elementBonds(char)
                st = i
...
def getBracketsParts(segmentName, startGrp,startBr,level,option):
    ...
    while char2=='(' and groupLength <= segmentName.__len__() - 1 :
        groupName = bracket(segmentName, groupLength)
        ...
        getStructure(groupName, times, level + 1,option)
...
def getStructure(condensedName,times,level,option):
    ...
    if isGroupExist(condensedName):
        if option=="getValancy":
            valancyGrp = groupBonds(condensedName)
        ...
        while i <=nameLength-1:
            ...
            if char =='-' or i == 0 or char =='=' or char =='-=':
                ...
                if char1 == '(':
                    ...
                    segmentName = getBracketsParts(condensedName,startGrp, i,level,"segmentName")

                    if isGroupExist(segmentName):
                         if option == "getValancy":
                            valancyGrp = groupBonds(segmentName)
                            print(levelSp,"Segment Name= ", segmentName, "Group Bonds= ", valancyGrp)
                    else:
                         segmentName=getBracketsParts(condensedName, startGrp, i, level,option)
                    ...
                    getStructure(segmentName,1,level+1,option)
                    startGrp = startGrp1
                    i= end1-1
...
def main():
    ...
    global level
    level = 1
    if checkBr(condensedName):
        getStructure(condensedName,1,level,"Check")
    else:
